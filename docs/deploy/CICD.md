...
- GitHub Actions path: `.github/workflows/deploy.yml` builds and deploys `out/` to Pages when Actions is available
- Manual GitHub Pages fallback: `pnpm run deploy:pages:branch` builds locally and pushes `out/` to the `gh-pages` branch

## Functions (Serverless)
- Production deploy: `pnpm run deploy:api:prod`
- Removal: `pnpm run remove:api:prod`
- Dev (local): `pnpm run dev:api`
...
- Run `pnpm run lint` as part of CI for quality checks
